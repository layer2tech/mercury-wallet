...
import {
  SWAP_STATECHAIN_INFO_AFTER_TRANSFER,
  TRANSFER_FINALIZED_DATA,
  TRANSFER_MSG_4,
  STATECOIN_AMOUNT,
  setSwapDetails,
  SWAP_INFO,
...
function get_statecoin_in() {
  ...
  tm3.tx_backup_psm.shared_key_id = cloneDeep(
    MOCK_SERVER.TRANSFER_RECEIVER.new_shared_key_id
  );
...
function get_statecoin_after_transfer_receiver(statecoin) {
  ...
  sc.swap_transfer_finalized_data.tx_backup_psm =
    sc.swap_transfer_msg_3_receiver.tx_backup_psm;
  return sc;
}
...
function get_statecoin_out_expected(statecoin_out, smt_proof = null) {
  ...
  let statecoin_out_expected = get_statecoin_after_transfer_receiver(
    get_statecoin_in()
  );
  ...
  statecoin_out_expected.anon_set =
    statecoin_out_expected.anon_set +
    SWAP_INFO.swap_token.statechain_ids.length;
  statecoin_out_expected.swap_transfer_finalized_data = null;
  statecoin_out_expected.status = STATECOIN_STATUS.AVAILABLE;
  statecoin_out_expected.shared_key_id =
    TRANSFER_FINALIZED_DATA.new_shared_key_id;
  statecoin_out_expected.statechain_id = TRANSFER_FINALIZED_DATA.statechain_id;
  statecoin_out_expected.funding_txid =
    SWAP_STATECHAIN_INFO_AFTER_TRANSFER.utxo.txid;
  ...
  (statecoin_out_expected.proof_key =
    ...
    (statecoin_out_expected.sc_address =
      "sc1q29fkeks6trw7ll5fggr63x5ay3zk8azl56v6h0znf2gwhz49275zn69cks"),
...
const proof_key_der_new = bitcoin.ECPair.fromPrivateKey(
...
describe("Swap phase 4", function () {
    ...
    try {
      ...
      for (let i = 0; i < STATECOIN_STATUS.length; i++) {
        if (STATECOIN_STATUS[i] !== STATECOIN_STATUS.IN_SWAP) {
          ...
          await expect(swapPhase4(swap)).rejects.toThrowError(
      ...
      for (let i = 0; i < SWAP_STATUS.length; i++) {
        if (SWAP_STATUS[i] !== SWAP_STATUS.Phase4) {
          ...
          swap = getSwap(wallet, statecoin);
          await expect(swapPhase4(swap)).rejects.toThrowError(
            `phase Phase4:transferReceiver: invalid swap status: ${statecoin.swap_status}`
          );
          expect(statecoin).toEqual(INIT_STATECOIN);

          swap = getSwap(wallet, statecoin);
          await expect(swapPhase4(swap)).rejects.toThrow(Error);
          expect(statecoin).toEqual(INIT_STATECOIN);
        }
      }
    ...
    try {
      ...
      for (let i = 0; i < SWAP_STATUS.length; i++) {
        ...
        if (phase !== SWAP_STATUS.Phase4 && phase !== null) {
          await expect(
            swapPhase4(http_mock, null, statecoin, null)
          ...
          await expect(
            ...
            `Swap error: Expected swap phase4. Received: ${phase}`
          );
          expect(statecoin).toEqual(EXPECTED_STATECOIN);
        }
      }
  ...
  test("swapPhase4 test 4 - error requesting keygen first in transferReceiverFinalize", async function () {
    try {
      ...
      updated_statecoin.swap_error = {
        error: true,
        msg: "statecoin c93ad45a-00b9-449c-a804-aab5530efc90 waiting for completion of batch transfer in swap ID c7207feb-c575-482f-8612-307c3d4cd133",
      };
  ...
  test("swapPhase4 test 6 - error requesting keygen second in transferReceiverFinalize", async function () {
    try {
      ...
      for (let i = 0; i < valid_phases.length; i++) {
        ...
        expect(result.is_ok()).toEqual(false);
        expect(result.message).toEqual(
          post_error(POST_ROUTE.KEYGEN_SECOND).message
        );
        copy_t2(statecoin, UPDATED_STATECOIN);
        expect(statecoin).toEqual(UPDATED_STATECOIN);
      }
  ...
  test("swapPhase4 test 8 - error client keygen second in transferReceiverFinalize", async function () {
    try {
      ...
      for (let i = 0; i < valid_phases.length; i++) {
        ...
        expect(result.is_ok()).toEqual(false);
        expect(result.message).toEqual(
          wasm_err("KeyGen.second_message").message
        );
        copy_t2(statecoin, UPDATED_STATECOIN);
        expect(statecoin).toEqual(UPDATED_STATECOIN);
      }
  ...
  test("swapPhase4 test 9 - error from client set master key in transferReceiverFinalize", async function () {
    try {
      ...
      for (let i = 0; i < valid_phases.length; i++) {
        ...
        expect(result.is_ok()).toEqual(false);
        expect(result.message).toEqual(
          wasm_err("KeyGen.set_master_key").message
        );
        copy_t2(statecoin, UPDATED_STATECOIN);
        expect(statecoin).toEqual(UPDATED_STATECOIN);
      }
    ...
    try {
      ...
      for (let i = 0; i < valid_phases.length; i++) {
        ...
        wallet.addStatecoinFromValues(
          "c93ad45a-00b9-449c-a804-aab5530efc90",
          SHARED_KEY_DUMMY,
          0.1,
        ...
        let statecoin_out_expected = get_statecoin_out_expected(
          swap.statecoin_out
        );
  ...
  test("swapPhase4 test 14 - error from client keygen first message transferReceiverFinalize - No data for identifier", async function () {
    try {
      ...
      for (let i = 0; i < valid_phases.length; i++) {
        ...
        expect(result.is_ok()).toEqual(false);
        expect(result.message).toEqual(
          `statecoin ${UPDATED_STATECOIN.shared_key_id} waiting for completion of batch transfer in swap ID ${UPDATED_STATECOIN.swap_id.id}`
        );
        copy_t2(statecoin, UPDATED_STATECOIN);
        expect(statecoin).toEqual(UPDATED_STATECOIN);
      }
  ...
  test("swapPhase4 test 16 - error from client keygen first message transferReceiverFinalize - batch transfer status: finalized", async function () {
    try {
      ...
      expect(result.is_ok()).toEqual(false);
      expect(result.message).toEqual(
        `keygen first error: statecoin ${statecoin.shared_key_id} - batch transfer complete for swap ID ${statecoin.swap_id.id}`
      );
      copy_t2(statecoin, UPDATED_STATECOIN);
      expect(statecoin).toEqual(UPDATED_STATECOIN);
    } catch (error) {}
  ...
  test("swapPhase4 test 18 - await transferReceiver, server responds with  error to getStateChain() in  transferReceiver()", async () => {
    try {
      ...
      let commitment_data = {
        ...
        nonce: [
          118, 94, 232, 150, 99, 240, 44, 21, 13, 91, 170, 84, 58, 234, 242,
          220, 184, 197, 137, 219, 179, 125, 111, 165, 233, 100, 228, 21, 79,
          170, 3, 238,
        ],
      };
  ...
  test("swapPhase4 test 20 - repetition of transferReceiver", async () => {
    try {
      ...
      const do_transfer_receiver = async (
        expected = TRANSFER_FINALIZED_DATA
      ) => {
  ...
  test("swapPhase4 test 22 - invalid statecoin status in swapPhase4PollSwap", async () => {
    try {
      ...
      statecoin.swap_transfer_finalized_data = swap = getSwap(
        wallet,
        statecoin
      );
      step = swapPhase4Steps(swap).filter(step_filter)[0];
