...
describe('ElectrsClient', function () {
    ...
    test('get', async function () {
        const actual = await ElectrsClient.get(client.endpoint, "test/path", "params")
    ...
    test('get with timeout', async function () {
        const timeout = 1234
        const actual = await ElectrsClient.get(client.endpoint, "test/path", "params", timeout)
        expect(actual).toEqual(response.data)
        expect(axios).toBeCalledWith({
            method: 'get',
            url: "tor_endpoint.onion/test/path/params",
    ...
    test('post', async function () {
        const actual = await ElectrsClient.post(client.endpoint, "test/path", "body")
        expect(actual).toEqual(response.data)
        expect(axios).toBeCalledWith({
            method: 'post',
    ...
    test('post with timeout', async function () {
        const timeout = 1234
        const actual = await ElectrsClient.post(client.endpoint, "test/path", "body", timeout)
        expect(actual).toEqual(response.data)
        expect(axios).toBeCalledWith({
            method: 'post',
            url: "tor_endpoint.onion/test/path",
...
describe('ElectrsClient timeout', function () {
    ...
    test('get', async function () {
        await expect(ElectrsClient.get(client.endpoint, "test/path", "params")).rejects.
            toThrow(Error(`Electrum API request timed out: ${timeout_err(TIMEOUT).message}`))
        expect(axios).toBeCalledWith({
            method: 'get',
            url: "tor_endpoint.onion/test/path/params",
            headers: { 'Accept': 'application/json' },
    ...
    test('post', async function () {
        await expect(ElectrsClient.post(client.endpoint, "test/path", "body")).rejects.
            toThrow(Error(`Electrum API request timed out: ${timeout_err(TIMEOUT).message}`))
        expect(axios).toBeCalledWith({
            method: 'post',
            url: "tor_endpoint.onion/test/path",
