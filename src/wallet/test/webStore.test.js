...
import { backupTx } from './data/backup_tx';
...
describe("Account Breaks - Storing and Loading wallets in Redux", () => {
  ...
  test('Backup Tx Stored incorrectly', function(){

    wallet.statecoins.coins[0].tx_backup = backupTx;

    mock_wallet.storage.storeWallet( wallet );
    let loaded_wallet = mock_wallet.storage.getWallet(wallet.name);
    const loaded_backup_tx = loaded_wallet.statecoins.coins[0].tx_backup

    loaded_wallet.statecoins = StateCoinList.fromCoinsArray(loaded_wallet.statecoins.coins)

    expect(loaded_backup_tx).toStrictEqual(loaded_wallet.statecoins.coins[0].tx_backup)
  })

  test('Backup Tx Stored correctly - Electron Store', function(){

    wallet.statecoins.coins[0].tx_backup = backupTx;

    mock_wallet.storage.store = new Store({ name: mock_wallet.storage.name }) 
