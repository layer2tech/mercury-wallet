...
describe("swapPhase3", () => {
    ...
    try {
      ...
      for (let i = 0; i < STATECOIN_STATUS.length; i++) {
        if (STATECOIN_STATUS[i] !== STATECOIN_STATUS.IN_SWAP) {
          ...
          await expect(swapPhase3(swap)).rejects.toThrowError(
            `phase Phase3:pollSwapPhase3: invalid swap status: ${statecoin.swap_status}`
          );
        }
      }
      ...
      await expect(swapPhase3(swap)).rejects.toThrowError(
        "No swap address found for coin. Swap address should be set in Phase1"
      );
      ...
      await expect(swapPhase3(swap)).rejects.toThrowError(
        "No receiver address found for coin. Receiver address should be set in Phase1"
      );
  ...
  test("swapPhase3 test 2 - SwapStep1: server responds to pollSwap with miscellaneous error", async () => {
    ...
    try {
      ...
      checkRetryMessage(await swapPhase3(swap), `${server_error().message}`);
  ...
  test("swapPhase3 test 3 - SwapStep1: server responds to pollSwap with null or invalid status", async () => {
    try {
      ...
      await expect(swapPhase3(swap)).rejects.toThrow(Error);
      await expect(swapPhase3(swap)).rejects.toThrowError(
        "Swap halted at phase 3"
      );
      ...
      for (let i = 0; i < SWAP_STATUS.length; i++) {
        ...
        if (phase !== SWAP_STATUS.Phase3) {
          ...
          await expect(swapPhase3(swap)).rejects.toThrow(Error);
          await expect(swapPhase3(swap)).rejects.toThrowError(
            `Swap error: Expected swap phase3. Received: ${phase}`
          );
        }
      }
  ...
  test("swapPhase3 test 7 - SwapStep2: an invalid data type is returned from request for POST.TRANSFER_SENDER in transferSender()", async () => {
    try {
      ...
      const transfer_missing_x1_error =
        'transferSender: Expected property "x1" of type Object, got undefined';
      ...
      checkRetryMessage(await swapPhase3(swap), transfer_missing_x1_error);
  ...
  test("swapPhase3 test 11 - SwapStep2: server responds with error to POST.TRANSFER_UPDATE_MSG in transferUpdateMsg()", async () => {
    try {
      ...
      expect(statecoin.swap_transfer_msg.t1.secret_bytes).not.toEqual(
        statecoin_expected.secret_bytes
      );
      statecoin.swap_transfer_msg.t1.secret_bytes =
        statecoin_expected.swap_transfer_msg.t1.secret_bytes;
      ...
      statecoin.swap_transfer_msg.statechain_sig =
        statecoin_expected.swap_transfer_msg.statechain_sig;
  ...
  test("swapPhase3 test 12 - SwapStep3: getTransferMsg3", async () => {
    try {
      ...
      await expect(swap.getTransferMsg3()).rejects.toThrowError(
        Error(`Error: ${GET_ROUTE.TRANSFER_GET_MSG_ADDR}`)
      );
      expect(swap.statecoin.swap_transfer_msg_3_receiver).toEqual(null);
      ...
      expect(
        result.includes("Transfer message 3 not found - retrying...")
      ).toEqual(true);
