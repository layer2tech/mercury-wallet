    ...
    test('swapPhase3 test 1 - invalid initial statecoin state', async () => {
        ...
        for (let i = 0; i < STATECOIN_STATUS.length; i++) {
            if (STATECOIN_STATUS[i] !== STATECOIN_STATUS.IN_SWAP) {
                ...
                await expect(swapPhase3(http_mock, null, null, statecoin, null, proof_key_der, proof_key_der, null, null, null)).rejects.toThrowError(`Coin status is not IN_SWAP. Status: ${sc_status}`)
            }
        }
        ...
        for (let i = 0; i < SWAP_STATUS.length; i++) {
            if (SWAP_STATUS[i] !== SWAP_STATUS.Phase3) {
                ...
                await expect(swapPhase3(http_mock, null, null, statecoin, null, proof_key_der, proof_key_der, null, null, null))
    ...
    test('swapPhase3 test 3 - server responds to pollSwap with null or invalid status', async () => {
        ...
        for (let i = 0; i < SWAP_STATUS.length; i++) {
            ...
            if (phase !== SWAP_STATUS.Phase3) {
                ...
                await expect(swapPhase3(http_mock, null, null, statecoin, null, proof_key_der, proof_key_der))
                    .rejects.toThrow(Error)
                await expect(swapPhase3(http_mock, null, null, statecoin, null, proof_key_der, proof_key_der))
