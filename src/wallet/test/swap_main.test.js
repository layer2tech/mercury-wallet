...
const swap_steps = (swap) => {
  return [
    new SwapStep(
      ...
      () => {
        if (
          swap.statecoin.statechain_id === null ||
          swap.statecoin.statechain_id === undefined
        )
          throw Error("statechain id is invalid");
        return true;
      },
      swap.pollUtxo
    ),
...
describe("Swap Checks", function () {
  ...
  test("test checkNRetries", function () {
    ...
    expect(() => {
      ...
      Error(`Number of tries exceeded in phase ${statecoin.swap_status}`)
    );
  });
  ...
  test("test checkSwapLoopStatus", () => {
    ...
    }).toThrow(Error("Coin removed from swap pool"));
  ...
  test("checkWalletStatus throws an Error if wallet is not active", () => {
    ...
    swap.swap_steps = swap_steps(swap);
    expect(() => {
      swap.checkWalletStatus();
    }).toThrow(Error("wallet unloading..."));
  });
  ...
  test("test checkStatecoinStatus", () => {
    ...
    expect(() => {
      ...
      Error(
        `${step.description()}: invalid statecoin status: ${statecoin.status}`
      )
    );
  });
  ...
  test("test checkSwapStatus", () => {
    ...
    expect(() => {
      ...
      Error(
        `${step.description()}: invalid swap status: ${statecoin.swap_status}`
      )
    );
  });
  ...
  test("validate swap", function () {
    ...
    let swap = new Swap(wallet, statecoin);
    let err = Error(
      "Coin " + statecoin.getTXIdAndOut() + " already involved in swap."
    );
    ...
    swap = new Swap(wallet, statecoin);
    err = Error(
      `Coin ${statecoin.shared_key_id} is in swap phase 4. Swap must be resumed.`
    );
    checkValidateSwap(swap, statecoin, err);
  ...
  test("validate resume swap", function () {
    ...
    swap.resume = true;
    let err = Error(
      "Cannot resume coin " + statecoin.shared_key_id + " - not in swap."
    );
...
describe("swapToken", function () {
  ...
  test("swapPhase4HandleErrPollSwap", async function () {
    ...
    expect(result.message).toEqual(
      `post:${POST_ROUTE.SWAP_POLL_SWAP}:${swap_id}`
    );
...
describe("Do Swap Poll", function () {
  ...
  let mock_proof_key_der = bitcoin.ECPair.fromPrivateKey(
  ...
  test("Preparing Statecoin for Swap", function () {
    ...
    swap.resume = false;
    expect(() => swap.prepare_statecoin()).toThrow(
      `Coin ${statecoin.shared_key_id} is in swap phase 4. Swap must be resumed.`
    );
...
describe("Deregister statecoin", function () {
  ...
  test("checkRemoveCoinFromSwapPool from statecoin wrong state is unsuccessful", async function () {
    ...
    expect(() => {
      wallet.statecoins.checkRemoveCoinFromSwapPool(wrong_id);
  ...
  test("checkRemoveCoinFromSwapPool from statecoin awaiting swap successful", async function () {
    ...
    let result;
    await expect(
      (result = wallet.statecoins.checkRemoveCoinFromSwapPool(
        statecoin.shared_key_id
      ))
    ).resolves;
  ...
  test("removeCoinFromSwapPool from statecoin wrong state is unsuccessful", async function () {
    ...
    //Statecoin not found in wallet
    const wrong_id = "wrong_id";
    await expect(wallet.removeCoinFromSwapPool(wrong_id)).rejects.toThrow(
      Error("No coin found with shared_key_id " + wrong_id)
    );
  });
  ...
  test("removeCoinFromSwapPool from statecoin awaiting swap successful", async function () {
    ...
    setSwapDetails(initStatecoin, "Reset");
    await expect(wallet.removeCoinFromSwapPool(statecoin.shared_key_id))
      .resolves;
    expect(statecoin).toEqual(initStatecoin);
...
describe("Process step result Retry", function () {
  ...
  test("processStepResultRetry sets or clears statecoin swap error", async function () {
    ...
    for (let i = 0; i < values.length; i++) {
      let ts = values[i];
      await swap.processStepResultRetry(
        SwapStepResult.Retry(ts),
        swapInit(swap)[0]
      );
...
describe("Quit Wallet Mid Swap", function () {
  ...
  beforeEach(async () => {
    ...
    torEndpointsMock = jest
      .spyOn(wallet, "set_tor_endpoints")
      .mockImplementation();
    electrumInitMock = jest
      .spyOn(wallet, "initElectrumClient")
      .mockImplementation();
    swapGroupMock = jest
      .spyOn(wallet, "updateSwapGroupInfo")
      .mockImplementation();
