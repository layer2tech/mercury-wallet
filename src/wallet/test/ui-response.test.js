...
async function testClickTime(toButtonName, fromButtonName = "Back") {
  ...
  await waitFor(() => expect(screen.getByText(toButtonName)).toBeTruthy(), {
    timeout: 10000,
  });
...
async function testClickTimeAverage(toButtonName, fromButtonName = "Back") {
  ...
  console.log(
    `testClickTimeAverage- result: ${toButtonName}: ${outAverage}: ${backAverage}`
  );
...
async function goToMainPage() {
  ...
  let optionSelected = options.filter(
    (option) => option.value == walletNameBackup
  );
...
describe("UI performance", function () {
  ...
  test('responsiveness of main buttons and "generate address"', async function () {
    ...
    await waitFor(
      () => expect(screen.getByText("GENERATE ADDRESS")).toBeTruthy(),
      { timeout: 10000 }
    );
    ...
    for (let i = 0; i < n_iter; i++) {
      ...
      await waitFor(
        () =>
          expect(screen.getByTestId("Receive address").textContent).not.toEqual(
            address1
          ),
        { timeout: 10000 }
      );
