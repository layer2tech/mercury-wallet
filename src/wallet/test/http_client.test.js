...
describe("HttpClient", function () {
  ...
  afterEach(() => {
    ...
    //expect(semaphoreSpy).toHaveBeenCalledTimes(0);
  });
...
describe("HttpClient timeout", function () {
  ...
  test("get", async function () {
    await expect(client.get("test/path", "params")).rejects.toThrow(
      Error(`Mercury API request timed out: ${timeout_err(TIMEOUT).message}`)
    );
    expect(axios).toBeCalledWith({
      method: "get",
      url: "tor_endpoint.onion/test/path/params",
  ...
  test("post", async function () {
    await expect(client.post("test/path", "body")).rejects.toThrow(
      Error(`Mercury API request timed out: ${timeout_err(TIMEOUT).message}`)
    );
    expect(axios).toBeCalledWith({
      method: "post",
...
describe("error handling", function () {
  describe("checkForServerError", function () {
    ...
    test("status err", function () {
      expect(() => {
        checkForServerError(status_err);
      }).toThrow(
        Error(`http status: ${status_err.status}, data: ${status_err.data}`)
      );
    });

    test("string error", function () {
      expect(() => {
        checkForServerError(err_string);
      }).toThrow(Error(err_string.data));
    });

    test("string error lower case", function () {
      expect(() => {
        checkForServerError(err_string_lc);
      }).toThrow(Error(err_string_lc.data));
    });

    test("rate limiter error", function () {
      expect(() => {
        checkForServerError(rate_limiter_error);
      }).toThrow(
        Error(
          "The server is currently unavailable due to a high request rate. Please try again."
        )
      );
    });
    ...
    test("error object", function () {
      expect(() => {
        checkForServerError(error_object);
      }).toThrow(Error(JSON.stringify(error_object.data.error)));
    });
  });
  ...
  describe("handlePromiseRejection", function () {
    ...
    test("timeout error", async function () {
      expect(() => {
        handlePromiseRejection(timeout_err, timeout_msg);
      }).toThrow(Error(`${timeout_msg}: ${timeout_err.message}`));
    });

    test("misc error", () => {
      expect(() => {
        handlePromiseRejection(misc_err, timeout_msg);
      }).toThrow(misc_err);
    });

    test("blank error", () => {
      expect(() => {
        handlePromiseRejection(blank_err, timeout_msg);
      }).toThrow(Error(blank_err));
    });
  });
});
