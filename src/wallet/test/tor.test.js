...
let tor_endpoints = {
  state_entity_endpoint:
    "http://u3fi7yqrkv7jp5vwoui3e5rlgasnokbzg5uc42eltmsqbdqcxudsqjad.onion, http://lmfcwtytaxvfy6t6e7eumka3cvg3p7mhuybdj7iiaasndcqpskp5iwad.onion ",
  swap_conductor_endpoint:
    "http://u3fi7yqrkv7jp5vwoui3e5rlgasnokbzg5uc42eltmsqbdqcxudsqjad.onion, http://lmfcwtytaxvfy6t6e7eumka3cvg3p7mhuybdj7iiaasndcqpskp5iwad.onion ",
...
describe.skip("Tor", function () {
  ...
  if (bSkip == false) {
    beforeAll(async () => {
      let resourcesPath = joinPath(
        dirname(rootPath),
        "mercury-wallet/resources"
      );
      ...
      ta_process = exec(
        `node ${tor_adapter_path} ${tor_cmd} ${torrc} ${user_data_path}`,
        {
          detached: true,
          stdio: "ignore",
        },
      ...
      expect(JSON.stringify(result)).toEqual(
        JSON.stringify({
          state_entity_endpoint: [
            "http://u3fi7yqrkv7jp5vwoui3e5rlgasnokbzg5uc42eltmsqbdqcxudsqjad.onion",
            " http://lmfcwtytaxvfy6t6e7eumka3cvg3p7mhuybdj7iiaasndcqpskp5iwad.onion ",
          ],
          swap_conductor_endpoint: [
            "http://u3fi7yqrkv7jp5vwoui3e5rlgasnokbzg5uc42eltmsqbdqcxudsqjad.onion",
            " http://lmfcwtytaxvfy6t6e7eumka3cvg3p7mhuybdj7iiaasndcqpskp5iwad.onion ",
  ...
  describe("Tor server integration", function () {
    test("tor server get", async function () {
      ...
      expect(JSON.stringify(result2)).toBe(
        JSON.stringify({
          address: "tb1qzvv6yfeg0navfkrxpqc0fjdsu9ey4qgqqsarq4",
          deposit: 0,
          withdraw: 50,
          interval: 6,
          initlock: 12960,
          wallet_version: "0.6.5",
          wallet_message: "",
        })
      );
    });
    ...
    test("tor server post success", async function () {
      ...
      let deposit_msg1 = {
        auth: "authstr",
        proof_key: String(
          "029e95d1c597bd15eed0cd5fc15db25368202fc538a3a94a0b44753595f3aa4fc7"
        ),
      };
    ...
    describe("electrs", function () {
      test("broadcast transaction", async function () {
        ...
        const rawTX =
          "";
        try {
          await electrsClient.broadcastTransaction(rawTX);
        } catch (err) {
          console.log(`*****bc: ${err}`);
          expect(
            JSON.parse(err.message).message.includes(
      ...
      test("get transaction", async function () {
        const electrsClient = new ElectrsClient("http://localhost:3001");
        const txid =
          "8197859978177a8f3cc41996cc59205eb9a7457e2a3bbd046e755ea71b3a6849";
        let response = await electrsClient.getTransaction(txid);
        expect(response).toEqual({
          txid: "8197859978177a8f3cc41996cc59205eb9a7457e2a3bbd046e755ea71b3a6849",
          version: 2,
          locktime: 2193559,
          vin: [
            {
              txid: "7f782b5a741cdbac855f10c533da213e601b887ad314607465e96d962a12f4f4",
              vout: 1,
              prevout: {
                scriptpubkey: "00146d75875954e6564f96ad5e1d80a3c5424c742e91",
                scriptpubkey_asm:
                  "OP_0 OP_PUSHBYTES_20 6d75875954e6564f96ad5e1d80a3c5424c742e91",
                scriptpubkey_type: "v0_p2wpkh",
                scriptpubkey_address:
                  "tb1qd46cwk25uetyl94dtcwcpg79gfx8gt53r3j0np",
                value: 9025000034,
              },
              scriptsig: "",
              scriptsig_asm: "",
              witness: [
                "304402204808b6478fe9a4e6ad74bf664a21ed6243493b7057d5af67735e9de84c5489a5022013cda421cbb9d8ddd826b67debfcfc364948ed3a084406545d5b235a9fa8044a01",
          ...
          vout: [
            {
              scriptpubkey:
                "76a9143ee50e81f9793779652efcdbdc20fe084a82915e88ac",
              scriptpubkey_asm:
                "OP_DUP OP_HASH160 OP_PUSHBYTES_20 3ee50e81f9793779652efcdbdc20fe084a82915e OP_EQUALVERIFY OP_CHECKSIG",
              scriptpubkey_type: "p2pkh",
              scriptpubkey_address: "mmFWb5pqUaKRaNJor7hF5var3tbAgieeDw",
              value: 285655,
            },
            {
              scriptpubkey:
                "76a914c954cc7eab07d293129b99652933c4d9189c18b688ac",
              scriptpubkey_asm:
                "OP_DUP OP_HASH160 OP_PUSHBYTES_20 c954cc7eab07d293129b99652933c4d9189c18b6 OP_EQUALVERIFY OP_CHECKSIG",
              scriptpubkey_type: "p2pkh",
              scriptpubkey_address: "mysVhY4gXeY1EGYAnKyLKNpvV17M5ZYUgL",
              value: 9024699400,
            },
          ],
          size: 228,
          ...
          status: {
            confirmed: true,
            block_height: 2193560,
            block_hash:
              "0000000000000026a0c8e16a6dbf7f954c93b7bd6b53e2a4307596efa6259f1b",
            block_time: 1649241800,
          },
        });
      });
    });
