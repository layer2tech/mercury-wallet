...
export class ChannelList {
  ...
  getChannel(proof_key: string) {
    let channel = this.channels.filter(
      (chan) => chan.funding?.proof_key === proof_key
    );
    return channel;
  }
  ...
  addChannelFunding(
    ...
    let channelFund = this.channels.filter(
      (channel) => channel.funding?.addr === addr
    );
  ...
  deleteChannel(addr: string) {
    let channelsLeft = this.channels.filter(
      (chan) => chan.funding?.addr !== addr
    );
