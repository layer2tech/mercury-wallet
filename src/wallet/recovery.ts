...
export const recoverCoins = async (
  wallet: Wallet,
  gap_limit: number,
  dispatch: any
): Promise<RecoveryDataMsg[]> => {
  ...
  addrs.push(addr);
  recovery_request.push({
    key: wallet.getBIP32forBtcAddress(addr).publicKey.toString("hex"),
    sig: "",
  });
  let count = 0;
  ...
  while (count < gap_limit) {
    for (let i = 0; i < NUM_KEYS_PER_RECOVERY_ATTEMPT; i++) {
      percentageComplete = Math.floor(
        (wallet.account.chains[0].k /
          (Math.ceil(gap_limit / NUM_KEYS_PER_RECOVERY_ATTEMPT) *
            NUM_KEYS_PER_RECOVERY_ATTEMPT)) *
          100
      );
    ...
    new_recovery_data_load = await getRecoveryRequest(
      wallet.http_client,
      recovery_request
    );
    recovery_request = [];
  ...
  wallet.electrum_client.importAddresses(
    addrs,
    wallet.getBlockHeight() - fee_info.initlock
  );

  return recovery_data;
};

export const getFinalizeDataForRecovery = async (
  wallet: Wallet,
  wasm: any,
  recovery_data: RecoveryDataMsg
): Promise<TransferFinalizeDataForRecovery> => {
  // make new function that return statechain id and does relevant check
  let sc_tf_data: TransferFinalizeDataAPI =
    await getStateChainTransferFinalizeData(
      wallet.http_client,
      recovery_data.statechain_id
    );
  let state_chain_data = await getStateChain(
    wallet.http_client,
    recovery_data.statechain_id
  );
  ...
  if (sig.purpose !== sc_tf_data.statechain_sig.purpose) {
    throw new Error(`expected statechain sig purpose ${JSON.stringify(
      sig.purpose
    )}, 
  ...
  if (!se_rec_addr_bip32) {
    throw new Error(
      `Key derivation for address ${back_up_rec_addr} not found in wallet`
    );
  }
...
export const groupRecoveredWithdrawalTransactions = (
  withdrawal_tx_map: Map<string, Set<string>>,
  ...
  withdrawal_tx_map.forEach((ids, txid) => {
    ...
    ids.forEach((id) => {
      ...
      if (statecoin != null) {
        statecoin.tx_withdraw_broadcast.push(
          new WithdrawalTxBroadcastInfo(
            tx_fee,
            tx,
            { shared_key_ids: ids_arr },
            rec_addr
          )
        );
      }
...
export const addRestoredCoinDataToWallet = async (
  wallet: Wallet,
  wasm: any,
  recoveredCoins: RecoveryDataMsg[]
) => {
  ...
  for (let i = 0; i < recoveredCoins.length; i++) {
    ...
    if (recoveredCoins[i].shared_key_data === "None") {
      let finalize_data_for_recovery = await getFinalizeDataForRecovery(
        wallet,
        wasm,
        recoveredCoins[i]
      );
      statecoin = await transferReceiverFinalizeRecovery(
        wallet.http_client,
        wasm,
        finalize_data_for_recovery
      );
    } else {
      let shared_key = JSON.parse(recoveredCoins[i].shared_key_data);
      // convert c_key item to be clinet curv library compatible
      shared_key.c_key = JSON.parse(
        wasm.convert_bigint_to_client_curv_version(
          JSON.stringify({ c_key: shared_key.c_key }),
          "c_key"
        )
      ).c_key;
      ...
      let master_key = {
        ...
        private: {
          x2: wallet
            .getBIP32forProofKeyPubKey(recoveredCoins[i].proof_key)
    ...
    if (statecoin) {
      if (recoveredCoins[i].statechain_id) {
        ...
        if (withdrawing != null && withdrawing != "None") {
          ...
          withdraw_tx_id_map.set(
            withdraw_transaction.getId(),
          ...
          withdrawal_addr_map.set(
            withdraw_transaction.getId(),
            withdrawing.rec_addr
          );
        } else {
        ...
        var se_key_share = secp256k1.keyFromPublic(
          statecoin.shared_key.public.p1,
          "hex"
        );
        var priv_key = secp256k1.keyFromPrivate(
          statecoin.shared_key.private.x2,
          "hex"
        );
