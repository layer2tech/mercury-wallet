...
export class ElectrsLocalClient {
  ...
  async connect(config = { protocol: "tcp", host: "127.0.0.1", port: "50001" }) {
  ...
  async latestBlockHeader(): Promise<any> {
    let header = await ElectrsLocalClient.get(this.endpoint, GET_ROUTE.LATEST_BLOCK_HEADER, {})
    return header
  }
  ...
  async broadcastTransaction(rawTX: string): Promise<string> {
    let result = ElectrsLocalClient.post(this.endpoint, GET_ROUTE.TX, { "data": rawTX })
    return result
  }

  async getFeeEstimation(num_blocks: number, net_config: string): Promise<any> {
    let result = ElectrsLocalClient.get(this.endpoint, GET_ROUTE.FEE_ESTIMATES, {}).then((histo) => [`${num_blocks}`])
    return result
  }
