...
export class ActivityLog {
  ...
  static fromJSON(activity_json: ActivityLog): ActivityLog {
    ...
    activity_json.items.forEach((item: ActivityLogItem) => {
      ...
      if (item?.statecoin_id) {
        delete Object.assign(item, { shared_key_id: item.statecoin_id })[
          "statecoin_id"
        ];
      }
  ...
  getDate(shared_key_id: string, action: string) {
    let item = this.items.filter(
      (item) => item.shared_key_id === shared_key_id && item.action === action
    );
    ...
    if (item.length === 0 && action === ACTION.WITHDRAW) {
      ...
      item = this.items.filter(
        (item) =>
          item.shared_key_id === shared_key_id &&
          item.action === ACTION.WITHDRAWING
      );
    }
  ...
  static mergeActivityByDate = (
    activity_data: ActivityLogItem[]
  ): ActivityLogItem[][] => {
