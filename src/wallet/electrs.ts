...
const checkForServerError = (return_val: any) => {
  ...
  if (
    typeof return_val === "string" &&
    (return_val.includes("Error") || return_val.includes("error"))
  ) {
...
export class ElectrsClient {
  ...
  async latestBlockHeader(): Promise<number> {
    let latesthash = await ElectrsClient.get(
      this.endpoint,
      GET_ROUTE.BLOCKS_TIP_HASH,
      {}
    );
    let header = await ElectrsClient.get(
      this.endpoint,
      `${GET_ROUTE.BLOCK}/${latesthash}/${GET_ROUTE.HEADER}`,
      {}
    );
    let info = await ElectrsClient.get(
      this.endpoint,
      `${GET_ROUTE.BLOCK}/${latesthash}`,
      {}
    );
    info.header = header;
    return info;
  ...
  async getTransaction(txHash: string): Promise<any> {
    let result = await ElectrsClient.get(
      this.endpoint,
      `${GET_ROUTE.TX}/${txHash}`,
      {}
    );
  ...
  async getScriptHashListUnspent(script: string): Promise<any> {
    ...
    let data: Array<any> = await ElectrsClient.get(
      this.endpoint,
      `${GET_ROUTE.SCRIPTHASH}/${scriptHash}/${GET_ROUTE.UTXO}`,
      {}
    );
  ...
  async getScriptHashStatus(
    ...
    let status = await ElectrsClient.get(
      endpoint,
      `${GET_ROUTE.SCRIPTHASH}/${scriptHash}`,
      {}
    );
  ...
  async getLatestBlock(callBack: any, endpoint: string): Promise<any> {
    //this.connect()
    try {
      let blockHeight = await ElectrsClient.get(
        endpoint,
        GET_ROUTE.BLOCKS_TIP_HEIGHT,
        {}
      );
