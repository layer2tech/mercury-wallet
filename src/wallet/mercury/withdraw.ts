...
export const withdraw = async (
  ...
  let [tx, msg2] = await withdraw_init(
    http_client,
    wasm_client,
    network,
    statecoins,
...
export const withdraw_init = async (
  ...
  for (const sc of statecoins) {
    ...
    let statechain: StateChainDataAPI = await getStateChain(
      http_client,
    ...
    if (chain_data !== statecoin.proof_key) {
      throw Error(
        "StateChain not owned by this Wallet. Incorrect proof key. Expected " +
          statecoin.proof_key +
          ", got " +
          chain_data
      );
  ...
  if (statecoins.length > 1) {
    txb_withdraw_unsigned = txWithdrawBuildBatch(
      network,
      sc_infos,
      rec_addr,
      fee_info,
    ...
    let withdraw_fee = Math.floor(
      (statecoin.value * fee_info.withdraw) / 10000
    );
  ...
  sc_infos.forEach((info, index) => {
    ...
    signatureHashes.push(
      getSigHash(tx_withdraw_unsigned, index, pk, info.amount, network)
    );
  });
  ...
  signatures.forEach((signature, index) => {
    ...
    tx_backup_signed.ins[index].witness = [
      Buffer.from(sig0),
      Buffer.from(sig1),
    ];
  });
...
export const withdraw_confirm = async (
  ...
  let signatures: any[][] = await http_client.post(
    POST_ROUTE.WITHDRAW_CONFIRM,
    withdraw_msg_2
  );
};
...
export const withdraw_duplicate = async (
  ...
  signatureHashes.push(
    getSigHash(tx_withdraw_unsigned, index, pk, statecoins[0].value, network)
  );
