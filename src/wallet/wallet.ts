...
export class Wallet {
  ...
  constructor(name: string, password: string, mnemonic: string, account: any, config: Config) {
    ...
    this.http_client = new HttpClient(this.config.swap_conductor_endpoint);
  ...
  async withdraw(
    ...
    shared_key_ids.forEach( (shared_key_id) => {
      ...
      if (statecoin.status!==STATECOIN_STATUS.AVAILABLE) throw Error("Coin "+statecoin.getTXIdAndOut()+" not available for withdraw.");
