...
export const verifySmtProof = async (
  wasm_client: any,
  root: Root,
  proof_key: string,
  proof: any
  ...
  return wasm_client.verify_statechain_smt(
...
export const pubKeyToScriptPubKey = (pub_key: string, network: Network) => {
  return bitcoin.address.toOutputScript(
    pubKeyTobtcAddr(pub_key, network),
    network
  );
};
...
export const toSatoshi = (btc: number) => {
  return Math.floor(btc * 10e7);
};
// Satoshi value -> BTC value
export const fromSatoshi = (sat: number) => {
  return sat / 10e7;
};
...
export const getSigHash = (
  ...
  return tx
    .hashForWitnessV0(index, script, amount, Transaction.SIGHASH_ALL)
    .toString("hex");
};
...
//     - amount-fee to receive address, and
//     - amount 'fee' to State Entity fee address
export const txWithdrawBuildBatch = (
  network: Network,
  sc_infos: Array<StateChainDataAPI>,
  rec_address: string,
  fee_info: FeeInfo,
  fee_per_byte: number
): TransactionBuilder => {
...
export const encodeSCEAddress = (
  proof_key: string,
  test_wallet: any = null
) => {
  let config;
...
export const encodeSecp256k1Point = (
  publicKey: string
): { x: string; y: string } => {
...
export const isValidNodeKeyAddress = (nodeKeyAddress: string) => {
  ...
  const regexIPv6 = /^([0-9a-fA-F]{66})@((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)|([0-9a-fA-F]{66})@((\[)?([0-9a-fA-F]{1,4}:){7,7}[0-9a-fA-F]{1,4}(\])?|(\[)?([0-9a-fA-F]{1,4}:){1,7}:([0-9a-fA-F]{1,4}(\])?)?|(\[)?([0-9a-fA-F]{1,4}:){1,6}:([0-9a-fA-F]{1,4}:){1,1}[0-9a-fA-F]{1,4}(\])?|(\[)?([0-9a-fA-F]{1,4}:){1,5}:([0-9a-fA-F]{1,4}:){1,2}[0-9a-fA-F]{1,4}(\])?|(\[)?([0-9a-fA-F]{1,4}:){1,4}:([0-9a-fA-F]{1,4}:){1,3}[0-9a-fA-F]{1,4}(\])?|(\[)?([0-9a-fA-F]{1,4}:){1,3}:([0-9a-fA-F]{1,4}:){1,4}[0-9a-fA-F]{1,4}(\])?|(\[)?([0-9a-fA-F]{1,4}:){1,2}:([0-9a-fA-F]{1,4}:){1,5}[0-9a-fA-F]{1,4}(\])?|(\[)?[0-9a-fA-F]{1,4}::([0-9a-fA-F]{1,4}:){1,6}[0-9a-fA-F]{1,4}(\])?|(\[)?::([0-9a-fA-F]{1,4}:){1,7}(\])?|(\[)?([0-9a-fA-F]{1,4}:){1,7}:([0-9a-fA-F]{1,4}(\])?)?)?:\d+$/;
  return regexIPv4.test(nodeKeyAddress) || regexIPv6.test(nodeKeyAddress);
}

export const isValidLnInvoice = (invoice: string) => {
  try {
    const decodedInvoice = bolt11.decode(invoice);
    return decodedInvoice.complete;
  } catch (error) {
    return false;
  }
}
