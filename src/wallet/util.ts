...
export const encryptECIES = (publicKey: any, data: any): Buffer => {
  log.debug('encryptECIES...')
  let data_arr = new Uint32Array(Buffer.from(zero_pad(data), "hex"));
  let result = ecies.encrypt(publicKey, Buffer.from(data_arr));
  log.debug('encryptECIES: return...')
  return result;
}

// ECIES decrypt string x1 from Server.
export const decryptECIES = (secret_key: any, encryption: any): string => {
  log.debug('decryptECIES...')
  let enc = encryption;
  //new Uint32Array(Buffer.from(encryption, "hex"))
  let dec = ecies.decrypt(secret_key, enc);
