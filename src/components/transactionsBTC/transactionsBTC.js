...
const TransactionsBTC = (props) => {
  ...
  let e2e_mode = require("../../settings.json").e2e_mode;
  // override this if we are in e2e mode
  if (e2e_mode) {
    testing_mode = false;
  }
  ...
  props.selectedValues.forEach((item, id) => {
    if (!item.initialised && item.value !== null) {
      setLoading(true);
      dispatch(callDepositInit(item.value)).then((res) => {
        // when finished update p_addr in GUI
        if (res.error === undefined) {
          props.setValueSelectionAddr(id, res.payload[1]);
          let new_deposit_inits =
            callGetUnconfirmedAndUnmindeCoinsFundingTxData();
          if (
            JSON.stringify(deposit_inits) !== JSON.stringify(new_deposit_inits)
          ) {
  ...
  const depositConfirm = () => {
    callGetUnconfirmedAndUnmindeCoinsFundingTxData().forEach((statecoin) => {
      dispatch(
        callDepositConfirm({ shared_key_id: statecoin.shared_key_id })
      ).then((res) => {
        if (res.error === undefined) {
          dispatch(
            setNotification({
              msg:
                "Deposit Complete! StateCoin of " +
                fromSatoshi(statecoin.value) +
                " BTC created.",
            })
          );
        }
  ...
  const populateWithTransactionDisplayPanels = deposit_inits.current.map(
    (item, index) => {
      if (item.value != null) {
        return (
          <div key={index}>
  ...
  return (
    <div className="Body">
      <div className="deposit-scan">
        {props.confirmations === -1 && (
          <QRCodeGenerator
            ...
            amount={fromSatoshi(props.amount)}
            level="H"
          />
        )}
        ...
        <div className="deposit-scan-content">
          ...
          <div className="deposit-scan-main">
            <div className="deposit-scan-main-item">
              ...
              <span>
                <b>{fromSatoshi(props.amount)}</b> BTC
              </span>
            </div>
            ...
            <div className="deposit-scan-main-item">
              {props.confirmations === -1 ? (
                <>
                  ...
                  <span className="long">
                    <b data-cy="deposit-address">{props.address}</b>
                  </span>
                </>
              ) : (
      ...
      <span className="deposit-text">
        Create statecoin by sending {fromSatoshi(props.amount)} BTC to the above
