...
const AutoSwapToggle = ({ coin_data }) => {
  ...
  const handleAutoSwap = async (item) => {
    ...
    // if the statecoin is not equal to the deposit value, do not allow to swap
    if (!statecoin.is_deposit_value_equal) {
      return;
    }
  ...
  return (
    (swapPage === "/swap_statecoin" || swapPage === "/home") && (
      <div>
        <label className="toggle">Auto-swap</label>
