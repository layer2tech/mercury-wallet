...
const WithdrawPage = () => {
  ...
  const withdrawButtonAction = async () => {
    ...
    dispatch(
      callWithdraw({
        shared_key_ids: selectedCoins,
        rec_addr: inputAddr,
        fee_per_byte: txFeePerB,
      })
    ).then((res) => {
      if (res.error === undefined) {
        ...
        dispatch(
          setNotification({
            msg:
              "Withdraw to " + inputAddr + " - transaction broadcast complete.",
          })
        );
      }
      if (res.error !== undefined) {
          ...
          {withdraw_txid === "" ? (
            <div className="loading-container">
              ...
              <div className="loading-txt">
                Loading Withdrawal Transaction ID...
              </div>
            </div>
          ) : (
            <div>
      ...
      <div className="container">
        <div className="Body withdraw">
          ...
          <h3 className="subtitle">
            ...
            <br />
            <b> {fromSatoshi(balance_info.total_balance)} BTC</b> as{" "}
            <b>{balance_info.num_coins}</b> {filterByMsg()}
          </h3>
        </div>
