...
const SwapPage = () => {
  ...
  const { torInfo, inSwapValues, swapLoad } = useSelector(
  ...
  const updateSwapInfo = (isMounted) => {
    if (isMounted === true) {
      ...
      let swap_groups_array = swap_groups_data
        ...
        : [];
      let sorted_swap_groups_entry = swap_groups_array.sort(
  ...
  const swapButtonAction = async () => {
    // if the deposit value has changed, do not allow to swap
    if (!selectedCoin.is_deposit_value_equal) {
      return;
    }
    ...
    dispatch(
      setOneOffMsg({
        key: "swap_punishment",
        msg: "WARNING! Exiting the wallet whilst a swap is live causes the swap to fail and coins to be temporarily banned from entering swaps.",
      })
    );
      ...
      <div className="container ">
        {disabled === true ? (
          ...
          <>
            <div className="swap">
              <PageHeader
                title="Swap Statecoins"
                svgIcon={swapIcon}
                subTitle="Swap statecoins to increase their anonymity"
              />
