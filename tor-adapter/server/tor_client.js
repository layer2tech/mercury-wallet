...
class TorClient {
  ...
  set(torConfig) {
    ...
    this.log(
      "info",
      `TorClient.set() - setting config to ${JSON.stringify(newTorConfig)}`
    );
  ...
  torIPC(commands) {
    this.log(
      "debug",
      `TorClient.torIPC - sending commands to tor control port: ${JSON.stringify(
        commands
      )}`
    );
  ...
  async startTorNode(tor_cmd, torrc) {
    ...
    execFile(
      tor_cmd,
      ["--hash-password", `${this.torConfig.controlPassword}`].concat(geo_args),
      (_error, stdout, _stderr) => {
        let hashedPassword = stdout.replace(/\n*$/, "");
        hashedPassword =
          "16:3F85DAF2A2A34032603235343E19ABBE3CB6BF03F1443984F21EEE749F";

        this.tor_proc = execFile(
          tor_cmd,
          [
            "-f",
            `${torrc}`,
          ...
          (error) => {
            if (error) {
              console.log("HashedControlPassword->", hashedPassword);
              console.log("SOCKS port ->", this.torConfig.port);
              console.log("DataDir", this.dataPath);
              throw error;
            }
          }
        );
        ...
        this.log(
          "info",
          `TorClient.startTorNode - started tor node with pid ${this.tor_proc.pid}`
        );
      }
    );
  }
  ...
  async isNodeRunning() {
    let result = false;
    await exec(
      `curl --socks5 ${this.torConfig.ip}:${this.torConfig.port} --socks5-hostname ${this.torConfig.ip}:${this.torConfig.port} -s https://check.torproject.org/ | cat | grep -m 1 Congratulations | xargs`,
      (_error, stdout, _stderr) => {
        console.log(stdout);
  ...
  async stopTorNode() {
    ...
    try {
      ...
      this.signal_proc(0);
      const message3 =
        "tor node process still running - sending kill signal...";
    ...
    throw new Error(
      `Unable to shut down tor node with process id ${this.tor_proc.pid}\n`
    );
  }
