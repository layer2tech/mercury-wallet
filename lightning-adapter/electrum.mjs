...
class ElectrumClient{
    ...
    async getBlockHeight(){
      console.log('Get Block Height...')
        let res = (await ElectrumClient.get("http://127.0.0.1:18443/rest/chaininfo.json")).data
        if(res){ 
          return res.blocks
        }
    }
    ...
    async getLatestBlockHeader(height){
      ...
      try{
        ...
        currentBlockHash = (await ElectrumClient.get(`http://127.0.0.1:18443/rest/blockhashbyheight/${height}.json`)).data.blockhash
      ...
      try{
        ...
        res  = (await ElectrumClient.get(`http://127.0.0.1:18443/rest/headers/1/${currentBlockHash}.hex`)).data
    ...
    async getTxIdData(txid){
      let res = (await ElectrumClient.get(`http://127.0.0.1:18443/rest/tx/${txid}.json`)).data
    ...
    static async get(endpoint,  timeout_ms = TIMEOUT) {
      ...
      static async post(endpoint, timeout_ms = TIMEOUT) {
        ...
        axios.post('http://polaruser:polarpass@127.0.0.1:18443/', options)
          .then(response => {
